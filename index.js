...
export const Config = z.object({
  ...
  timeoutMs: z.number().step(1).min(1000).max(6000000).default(6000000),
  ...
  visionTaskTimeoutMs: z.number().step(1).min(1000).max(6000000).default(6000000),
  ...
  ocrTimeoutMs: z.number().step(1).min(1000).max(6000000).default(6000000),
...
export async function buildInstantLocalMap(ctx, messages, provider, options = {}) {
  ...
  const budgetMs =
    Number.isFinite(options.timeoutMs) && options.timeoutMs > 0 ? options.timeoutMs : 6000000
...
export function apply(ctx, config = {}, runtime = {}) {
  ...
  const timeoutMs = () => {
    ...
    return Number.isFinite(value) && value > 0 ? value : 6000000
  }
